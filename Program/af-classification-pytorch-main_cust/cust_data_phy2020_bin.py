...
# list files
data_list = glob.glob("../../Data/phy2020_train_20230426_0_mpvc_sel/*.mat")
...
# read excel
ref_df = pd.read_excel("../../DataBase/PHY2020/PHY2020/REFERENCE_OUT_CPSCEX_GEO_PTBXL.xlsx")
...
for k, d in dataset.items():
    ...
    for idx in ref_df.index:
        ...
        if k == tmp_record:
            ...
            if not multi_hit:
                if cls0_val == 1 and cls0_cnt < 3569:
                    # cut data in fixed length
                    data_src = dataset[k]['ecg']
                    data_len = len(data_src)
                    data_tmp = np.zeros(DATA_FIX_LEN)
                    ...
                    # file_list.append(k)
                    target_list.append(1)  # normal/abnormal
                    file_list.append(k)
                    cls2_cnt += 1
...
df.to_json("df_data/cust_data_phy2020_bin_.json", orient='records', lines=True)
